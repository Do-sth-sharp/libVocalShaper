...
#define VS_TRACKPARAM_SELECT (target->track == -1) \
? TrackDAO::getParam(ProjectDAO::getMasterTrack(ptr), target->param) \
: TrackDAO::getParam(ProjectDAO::getTrack(ptr, target->track), target->param)
...
namespace vocalshaper {
	namespace actions {
		namespace trackparam {
			...
			VS_CREATE_OBJECT_SELECT_METHOD_BY_ACTION(selectParam, Param, TrackParamAction, VS_TRACKPARAM_SELECT);
