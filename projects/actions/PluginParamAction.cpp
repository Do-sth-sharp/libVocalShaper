...
#define VS_PLUGINPARAM_SELECT (target->track == -1) \
? PluginDAO::getParam(TrackDAO::getPlugin(ProjectDAO::getMasterTrack(ptr), target->plugin), target->param) \
: PluginDAO::getParam(TrackDAO::getPlugin(ProjectDAO::getTrack(ptr, target->track), target->plugin), target->param)
...
namespace vocalshaper {
	namespace actions {
		namespace pluginparam {
			...
			VS_CREATE_OBJECT_SELECT_METHOD_BY_ACTION(selectParam, Param, PluginParamAction, VS_PLUGINPARAM_SELECT);
